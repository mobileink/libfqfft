...
DEBUG_FLAGS = select({
    "@libfqfft//bzl/config:enable_debug": ["-g"],
    "//conditions:default": ["-g0"]
}) + select({
    "//:with_verbose": ["-v"],
    "//conditions:default": []
}) + select({
    "@libfqfft//bzl/config:macos_disable_debug": ["-UDEBUG"],
    "//conditions:default": []
})
...
####    DEFINES    ####
DDEBUG = select({
    "@libfqfft//bzl/config:enable_debug": ["DEBUG"],
    "//conditions:default": ["NDEBUG"]
})

UDEBUG = select({
    "@libfqfft//bzl/config:macos_no_debug": ["-UDEBUG"],
    "//conditions:default": []
})
